...
Annihilatoren. Also in \(\Ass(M)\) nach ???
\end{proof}
...
\end{proof}
\begin{Satz} Sie \(A,\frakm\) lokaler Integritätsbereich und \(\frakm=(t)\) für ein \(t\neq 0\).
Wenn \[\bigcap_{n=1}^\infty(t^n)=0\] Dann gilt
\begin{enumerate}
\item Jedes \(0\neq x\in A\) hat die Form \(x=t^nu\) mit \(n\geq 0\) und \(u\) Einheit.
\item Definiere \(\nu(x)=n\) wenn \(x=t^nu\) und \(\nu(\frac x y )=\nu(x)-\nu(y)\) für alle \(\frac x,y \in \Quot(A)\).
Dann ist \(nu\) diskrete Berwertung mit DBR \(A\).
\item Jedes \(I\neq o\) hat die Form \(I=(t^n)\).
\end{enumerate}
...
\begin{enumerate}
\item[]
\item wenn \(x\in (t^n)\setminus (t^{n+1})\) dann ist \(x=t^n\¢dot u\) mit \(u\neq\frakm\) also Einheit
\item Klar da \(\nu(t^n)=n\in\ZZ\).
\item Sei \(n=\min\{k\mid t^n\in I\}\). Dann ist \(I=(t^n)\).
\end{enumerate}
\end{proof}
...
\begin{Satz} \(A\) ist Bewertungsrings genau dann, wenn \(>\) totale Ordnung auf \(\Gamma\) ist.
Falls das gilt ist \(\nu\colon K^*\to\Gamma\) eine Abbildung sodass
\begin{enumerate}
\item \(\nu(xy)=\nu(x)+\nu(y)\)
\item \(\nu(x+y)=\min\{\nu(x),\nu(y)\}\)
\end{enumerate}
Insebsondere wenn \(\nu\colon K^*\to \Gamma\) eine surjektive Abbildung wie oben ist und \(\Gamma\) geordnete Gruppe (dh Ordnung stabil under \(+\)),
Dann ist \(A=\{a\in K\mid \nu(a)\geq 0\}\cup\{0\}\) Bewertungsring und \(\Gamma=K^*/A^*\).


\end{Satz}
\begin{proof}
\(>\) ist totale Ordnung wenn einer der drei Fälle gilt
\begin{enumerate}
\item \(\frac a b \in A\) aber \(\frac b a \not\in A\)
\item \(\frac a b \in A^*\)
\item \(\frac b a \in A \) aber \(\frac a b \not\in A\).
\end{enumerate}
\end{proof}
