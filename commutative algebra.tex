...
\begin{Bsp}
    \begin{enumerate}
        ...
        \(p=\Ann(x)\). Somit \(Ass(M)=\Spec(\ZZ)\setmins\{0\}=\Supp(M).\)
...
\end{proof}
\begin{Lemma}
    Sei \(R\) ein Bewertungsring mit maximalem Ideal \(\frakm\) und sei \(A\subbseteq R/\frakm\) ein Bewertungsring
...
\begin{proof}
    ...
    Da \(Tr_{L/K}=Tr_{K(\alpha)/K}\circ Tr_{L/K(\alphha)}\) ist ohne Einschränkung \(L=K(\alpha)\).
    ...
    In \(\bar K\) gilt \(\tilde P(X)=(X-\beta_1)\dotts (X-\beta_d)\) für paarweise verschiedene 
    \(\beta_i\). 
    Dann ist 
...
\begin{Lemma}
    ...
    Dann ist \(B\) normal und für alle \(x\in L\) gilt \(x=\rac b a\) für ein \(b\in B\) und 
    \(a\in A\).
\end{Lemma}
\begin{proof}
...
\(y=ax+by^2\) in \(R_\frakp\) und somit ist \[(1-by)y=ax\in (x)R_\frakp\] Also ist \(y\in (x)R_\frakp\) und \((x)R_\frakp=\frakp_1R_\frakp=\frakpR_\frakp\).
...
Sei \(xI=\frakp_1\dots\đrakp_r\) Primzerlegung.
