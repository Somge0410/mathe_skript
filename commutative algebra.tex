...
Nach \nameref{Satz:LivingOver} existiert \( \frakq \subseteq R'/ \frakq_0\) über \( \bar{\frako_0} \).
...
\begin{enumerate}
\item \( \ht(\frakp)=\dim(R_\frakp) \) für ein Primideal \( \frakp \) von \( R \) und \(\coht(I)=\dim(R/I)\)
...
Außerdem ist
\begin{align*}
d & = \transgrad_K(\Quot(K[X_1,\dots,X_d]))\\
&= \ŧransgrad_K(\Quot(A))
...
Wähle \( y \in \frakp \cap K[x_1,\dots,x_d] \) mit \( y \neq 0 \). Das existiert da
 ...
 \end{proof}
 \begin{Bem} Es gilt \( \ht(\frakp)=\dim(R_\frakp)=\transdeg_K(\Quot(R_\frakp))=\transdeg_K(\Quot(A))\) Nach Satz ???
